...
The page uses system fonts and has no external assets and a small inline script for click interactions. Navigation links lead to fictional Home, About, Projects, and Contact sections and use two columns on small screens. Following a navigation link focuses its section for keyboard users, including when JavaScript is disabled. Four keyboard-accessible balls in blue, pink, green, and amber bounce slowly behind the greeting, taking 12 seconds in each direction. Hovering or focusing a ball pauses its background travel to make activation easier. Click a ball (or press Enter or Space when focused) for an extra bounce. Background travel stays paused until that bounce finishes, even if the pointer or focus moves away. The animation is disabled when the visitor prefers reduced motion. The questionnaire has been removed.
